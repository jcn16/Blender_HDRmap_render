...
def render_all_color(outpath, cam=None, obj_names=None, alpha=True, text=None):
    """Renders current scene with cameras in scene.
    ...
    # render albedo
    albedo_node = scene.node_tree.nodes.new('CompositorNodeOutputFile')
    albedo_node.base_path = ''
    bpy.context.scene.render.layers['RenderLayer'].use_pass_diffuse_color = True
    result_albedo = scene.node_tree.nodes['Render Layers'].outputs['DiffCol']
    scene.node_tree.links.new(result_albedo, albedo_node.inputs['Image'])
    albedo_node.format.file_format = 'PNG'
    albedo_node.format.color_depth = '8'
    albedo_node.format.color_mode = 'RGB'

    # render ambient
    ambient_node = scene.node_tree.nodes.new('CompositorNodeOutputFile')
    ambient_node.base_path = ''
    bpy.context.scene.render.layers['RenderLayer'].use_pass_ambient_occlusion = True
    result_ambient = scene.node_tree.nodes['Render Layers'].outputs['AO']
    scene.node_tree.links.new(result_ambient, ambient_node.inputs['Image'])
    albedo_node.format.file_format = 'PNG'
    albedo_node.format.color_depth = '8'
    albedo_node.format.color_mode = 'RGB'
...
def render_lighting_passes(outpath, cam=None, obj_names=None, n_samples=64):
    ...
    for p in select_passes:
        print(nodes['Render Layers'].outputs.keys())
        setattr(render_layer, 'use_pass_' + p, True)

    for p_key in select_passes_keys:
        # Set alpha
        set_alpha_node = nodes.new('CompositorNodeSetAlpha')
        node_tree.links.new(nodes['Render Layers'].outputs['Alpha'],
                            set_alpha_node.inputs['Alpha'])
...
def render_ambient(outpath, cam=None, obj_names=None, n_samples=64):
    """Renders ambient pass.
    ...
    # Enable all passes of interest
    bpy.context.scene.render.layers['RenderLayer'].use_pass_ambient_occlusion = True
    result_socket = scene.node_tree.nodes['Render Layers'].outputs['AO']

    # Render
    exr = outpath.endswith('.exr')
    outpath = _render(scene, outnode, result_socket, outpath,exr=exr)
...
def render_albedo(outpath, cam=None, obj_names=None, n_samples=64):
    ...
    # Enable all passes of interest
    bpy.context.scene.render.layers['RenderLayer'].use_pass_diffuse_color = True
    result_socket = scene.node_tree.nodes['Render Layers'].outputs['DiffCol']
...
def render_glossy(outpath, cam=None, obj_names=None, n_samples=64):
    """Renders albedo pass.
    ...
    result_socket = scene.node_tree.nodes['Render Layers'].outputs['GlossCol']
