...
def rotate_object(rotate_angle,rotate_axis=(False, False, False)):
    """
    Only rotate ONE object, encure there only contains ONE mesh in the scene
    """
    for o in bpy.data.objects:
        o.select = o.type in ('MESH')
        if o.select:
            print(f'Rotate the obejct:{o.name}')

    # rotate the obj
    bpy.ops.transform.rotate(value=rotate_angle,constraint_axis=rotate_axis,constraint_orientation='LOCAL',)

def scale_object(scale=(1.0, 1.0, 1.0),scale_axis=(False, False, False)):
    """
    Only rotate ONE object, encure there only contains ONE mesh in the scene
    """
    for o in bpy.data.objects:
    ...
    # rotate the obj
    bpy.ops.transform.resize(value=scale, constraint_axis=scale_axis, constraint_orientation='LOCAL')
...
def smart_uv_unwrap(obj, area_weight=0.0, angle_limit=66.):
    ...
    Args:
        ...
        area_weight (float, optional): "Weight projections vector by faces with
            larger areas".
        angle_limit (float, optional): "Lower for more projection groups, higher
            for less distortion"
