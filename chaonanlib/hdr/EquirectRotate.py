...
class EquirectRotate:
  """
    ...
    @:param rotation: x, y, z degree to rotate, x means longtitude
  """
  ...
  def __init__(self, height: int, width: int, rotation: tuple):
    ...
    # resize to quick compute
    self.src_Pixel=np.reshape(self.src_Pixel,(-1,2))
    self.src_Pixel_idx=np.zeros(self.src_Pixel.shape[0])
    self.src_Pixel_idx=self.src_Pixel[:,0]*self.width+self.src_Pixel[:,1]
  ...
  def rotate(self, image):
    ...
    rotated_img=np.reshape(rotated_img,(-1,3))
    image=np.reshape(image,(-1,3))
    rotated_img=image[self.src_Pixel_idx]
    rotated_img=np.reshape(rotated_img,(self.height, self.width,3))
    return rotated_img
