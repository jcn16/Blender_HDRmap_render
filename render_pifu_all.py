...
def main():
    ...
    config=read_ini('./config_pifu_all.ini')
    if not os.path.exists(config.get('DEFAULT', 'save_path')):
        os.makedirs(config.get('DEFAULT', 'save_path'))

    # model path
    with open(os.path.join(config.get('DEFAULT', 'model_json_path'),'train_0.json'),'r') as f:
    ...
    for model in child_models:
        ...
        for sub in child_dirs:
            # if there
            if os.path.exists(os.path.join(config.get('DEFAULT', 'save_path'), model,sub, 'raytracing.png')):
                continue

            # loading obj
            obj_path = os.path.join(config.get('DEFAULT', 'save_path'), model,sub, 'pifu.obj')
